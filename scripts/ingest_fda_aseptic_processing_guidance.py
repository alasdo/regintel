...
def fetch_pdf_text() -> str:
    ...
    if not full_text:
        raise ValueError("Failed to extract text from the FDA aseptic processing PDF.")
...
def extract_guidance_text(full_text: str) -> str:
    ...
    if not extracted:
        raise ValueError("Could not extract main aseptic processing guidance text.")
